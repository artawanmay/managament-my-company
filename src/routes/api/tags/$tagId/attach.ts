      ...
      POST: async ({ request, params }) => {
        ...
        try {
          ...
          if (existingTaggable.length > 0) {
            ...
            return json({ data: existingTaggable[0], message: 'Tag already attached' });
          }
          ...
          const result = await db.insert(taggables).values(taggableData).returning();
          ...
          if (error instanceof Error && error.message.includes('UNIQUE constraint failed')) {
            return json({ error: 'Tag is already attached to this entity' }, { status: 409 });
          }
