      ...
      PUT: async ({ request, params }) => {
        ...
        try {
          ...
          if (
            ...
            parsed.data.projectId !== note.projectId
          ) {
          ...
          if (
            ...
            parsed.data.clientId !== note.clientId
          ) {
            if (parsed.data.clientId) {
              ...
              if (!isSuperAdmin) {
                return json(
                  { error: "Access denied to set client notes" },
                  { status: 403 }
                );
              }
            }
          ...
          if (parsed.data.systemName !== undefined)
            updateData.systemName = parsed.data.systemName;
          ...
          if (parsed.data.host !== undefined)
            updateData.host = parsed.data.host;
          if (parsed.data.port !== undefined)
            updateData.port = parsed.data.port;
