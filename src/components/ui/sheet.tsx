  ...
  "fixed z-50 gap-4 glass-modal p-6 transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        ...
        left: "inset-y-0 left-0 h-full border-r data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left rounded-r-2xl rounded-l-none w-full sm:max-w-lg",
        right:
          "inset-y-0 right-0 h-full border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right rounded-l-2xl rounded-r-none w-full sm:max-w-lg",
      },
    },
    defaultVariants: {
      side: "right",
...
const SheetFooter = ({
  ...
  <div
    className={cn(
      "flex-shrink-0 flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 pt-4 border-t",
      className
    )}
    {...props}
  />
